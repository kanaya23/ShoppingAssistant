...
const GeminiWebAPI = {
    ...
    async sendMessage(messages, onChunk, onToolCall, onProgress, onToolResult) {
        ...
        // Clean up: close the Gemini tab after query completes
        if (this.tabId) {
            try {
                console.log(`[GeminiWeb] Closing Gemini tab ${this.tabId} (query complete)`);
                await browser.tabs.remove(this.tabId);
                this.tabId = null;
            } catch (e) {
                console.log('[GeminiWeb] Tab already closed or error closing:', e.message);
            }
        }

        // Also close the Shopee worker tab
        if (this.workerTabId) {
            try {
                console.log(`[GeminiWeb] Closing Shopee worker tab ${this.workerTabId} (query complete)`);
                await browser.tabs.remove(this.workerTabId);
                this.workerTabId = null;
            } catch (e) {
                console.log('[GeminiWeb] Worker tab already closed or error closing:', e.message);
            }
        }
    ...
    async executeTool(name, args, onProgress) {
        ...
        // Order: (name, current, total, url) to match what background.js expects
        const progressCallback = onProgress ? (current, total, url) => onProgress(name, current, total, url) : null;
        try {
