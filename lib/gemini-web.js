...
const GeminiWebAPI = {
    ...
    // Track the original Shopee tab for tool execution
    originalTabId: null,
    ...
    async sendMessage(messages, onChunk, onToolCall, onProgress, onToolResult) {
        ...
        // 2. Remember the ORIGINAL tab before switching to Gemini
        const currentTabs = await browser.tabs.query({ active: true, currentWindow: true });
        if (currentTabs.length > 0 && !currentTabs[0].url?.includes('gemini.google.com')) {
            this.originalTabId = currentTabs[0].id;
            console.log(`[GeminiWeb] Remembered original tab: ${this.originalTabId}`);
        }

        // 3. Ensure Gemini Tab is Open/Ready
        await this.ensureTab();

        // 4. Tool Loop with state tracking
        ...
        while (turnCount < MAX_TURNS) {
            ...
            let toolResult;
            try {
                // Switch to Shopee tab
                if (this.originalTabId) {
                    console.log(`[GeminiWeb] Switching to Shopee tab...`);
                    await browser.tabs.update(this.originalTabId, { active: true });
                ...
                if (this.tabId) {
                    console.log(`[GeminiWeb] Switching back to Gemini tab...`);
                    await browser.tabs.update(this.tabId, { active: true });
                    await new Promise(r => setTimeout(r, 1000)); // Wait for tab to be ready
                }
    ...
    async executeTool(name, args, onProgress) {
        ...
        // Ensure we have a valid Shopee tab
        await this.ensureShopeeTab();

        // Make sure the tab is active and content script is ready
        if (this.originalTabId) {
            await browser.tabs.update(this.originalTabId, { active: true });
            await this.waitForContentScript(this.originalTabId);
        }

        console.log(`[GeminiWeb] Executing tool '${name}' with tabId=${this.originalTabId}`, args);

        // Pass progress callback to ToolExecutor
        const progressCallback = onProgress ? (current, total) => onProgress(current, total, name) : null;
        const result = await ToolExecutor.execute(name, args, this.originalTabId, progressCallback);
        return result;
    },

    /**
     * Ensure a Shopee tab exists and is ready
     */
    async ensureShopeeTab() {
        // First check if originalTabId is still valid
        if (this.originalTabId) {
            try {
                const tab = await browser.tabs.get(this.originalTabId);
        ...
        const shopeeTabs = await browser.tabs.query({ url: '*://*.shopee.co.id/*' });
        if (shopeeTabs.length > 0) {
            this.originalTabId = shopeeTabs[0].id;
            console.log(`[GeminiWeb] Found existing Shopee tab: ${this.originalTabId}`);
            return;
        }

        // Create new Shopee tab
        console.log('[GeminiWeb] No Shopee tab found, creating one...');
        const newTab = await browser.tabs.create({ url: 'https://shopee.co.id', active: true });
        this.originalTabId = newTab.id;
        ...
        await new Promise((resolve, reject) => {
            ...
            const listener = (tabId, changeInfo) => {
                if (tabId === this.originalTabId && changeInfo.status === 'complete') {
